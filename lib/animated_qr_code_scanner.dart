...
import 'package:barcode_detector/perspective_trasform.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';

import 'package:qr_code_scanner/qr_code_scanner.dart';

import 'animated_qr_view_controller.dart';
...
class _AnimatedQRViewState extends State<AnimatedQRView> {
  ...
  QRViewController get _effectiveController =>
  ...
  final GlobalKey<AnimatedSquareState> animatedKey =
      GlobalKey<AnimatedSquareState>();
  ...
  void initState() {
    ...
    WidgetsBinding.instance?.addPostFrameCallback((_) => setState(() {
          final Size widgetSize =
              (parentQrKey.currentContext?.findRenderObject() as RenderBox)
                  .size;
          viewFinderSize = Size(
            widgetSize.width,
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scannedString) async {
      print(scannedString.code);
      // detector.bitMatrix = BitMatrix.fromString(scannedString.);
      //   if (defaultTargetPlatform == TargetPlatform.android) startHighlightQR();
      // controller.pauseCamera();
      // animatedKey.currentState?.setScanResult(scannedString.toString());
      // setState(() {
      //   qrText = scannedString.toString();
      //   if (defaultTargetPlatform == TargetPlatform.android) {
      //     widget.onScanBeforeAnimation(scannedString.toString());
      //   }
      // });
      // if (defaultTargetPlatform == TargetPlatform.iOS) {
      //   setState(() {
      //     widget.onScan(scannedString.toString());
      //   });
      // }
    });
    ...
    // controller.scannedDataStream.listen((scannedPointsString) async {
    //   resultPointsPreviewFramingRect =
    //       _parseListPoint(scannedPointsString.toString());
    // });
    ...
    // controller.previewFramingRectStream
    //     .listen((previewFramingRectString) async {
  ...
  void startHighlightQR() {
    final PerspectiveTransform transform = detector.createTransform(
        ...
        resultPointsPreviewFramingRect[3]);

    qrCornersPreviewFramingRect = transform.transformPointsFromOffset([
      const Offset(0, 0),
      Offset(detector.computedDimension.toDouble(), 0),
      Offset(detector.computedDimension.toDouble(),
          detector.computedDimension.toDouble()),
    ...
    Offset translateResultPointFramingRectToPreview(Offset point) {
      final double x = previewMirrored
          ? previewSize.width / 2 -
              (point.dx + previewFramingRect.left) // TODO: incorrect
    ...
    qrCornersPreview = [
      for (int i = 0; i < qrCornersPreviewFramingRect.length; i++)
        translateResultPointFramingRectToPreview(Offset(
            qrCornersPreviewFramingRect[i].dx,
            qrCornersPreviewFramingRect[i].dy))
    ];

    final Size scaledSize = Size(viewfinderRect.right - viewfinderRect.left,
        viewfinderRect.bottom - viewfinderRect.top);
    final Offset previewCenter =
        Offset(previewSize.width / 2, previewSize.height / 2);
    final Offset scaledCenter =
        Offset(scaledSize.width / 2, scaledSize.height / 2);

    final List<Offset> qrCornerScaled = [
      for (int i = 0; i < qrCornersPreview.length; i++)
        Offset(
          ((qrCornersPreview[i].dx - previewCenter.dx) *
                  scaledSize.width /
                  previewSize.width) +
              scaledCenter.dx,
          ((qrCornersPreview[i].dy - previewCenter.dy) *
                  scaledSize.height /
                  previewSize.height) +
              scaledCenter.dy,
        )
    ...
    final Size centerCroppedSize = previewSize.width / previewSize.height <
            viewFinderSize.width / viewFinderSize.height
        ? Size(
            scaledSize.width,
    ...
    final List<Offset> qrCornerCenterCropped = [
      for (int i = 0; i < qrCornerScaled.length; i++)
        Offset(
          qrCornerScaled[i].dx -
              (scaledSize.width - centerCroppedSize.width) / 2,
          qrCornerScaled[i].dy -
              (scaledSize.height - centerCroppedSize.height) / 2,
        )
    ];
    ...
    qrCornerFlutter = [
      for (int i = 0; i < qrCornerCenterCropped.length; i++)
        Offset(
          ...
          qrCornerCenterCropped[i].dy * previewToFlutterRatio.dy +
