...
class AnimatedSquareState extends State<AnimatedSquare>
  ...
  TickerFuture revertOffset() {
    ...
    final double top =
        (widget.widgetSize.height - widget.height) / 2 + widget.padding.top;
    final double right =
        (widget.widgetSize.width - widget.width) / 2 + widget.width;
    final double bottom = (widget.widgetSize.height - widget.height) / 2 +
        widget.height +
        widget.padding.top;
    ...
    setState(() {
      ...
      for (Animation<Offset> animation in animations) {
        animation.removeStatusListener(_statusListenerIdle);
      }
      animations = newOffset.map((offset) {
        final int i = newOffset.indexOf(offset);
        final Offset fromOffset = offsets[i];
  ...
  void initAnimation() {
    ...
    double bottom = (widget.widgetSize.height - widget.height) / 2 +
        widget.height +
        widget.padding.top;
    ...
    bottom = (widget.widgetSize.height - widget.height) / 2 +
        widget.height * tween +
        widget.padding.top;
    left = (widget.widgetSize.width - widget.width) / 2 +
        widget.width * (1 - tween);
    top = (widget.widgetSize.height - widget.height) / 2 +
        widget.height * (1 - tween) +
        widget.padding.top;
