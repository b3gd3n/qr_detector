...
class Detector {
  ...
  PerspectiveTransform createTransform(
    ...
    computedDimension = computeDimension(topLeft, topRight, bottomLeft,
  ...
  int computeDimension(
      Offset topLeft, Offset topRight, Offset bottomLeft, double moduleSize) {
    final int tltrCentersDimension =
        (topLeft - topRight).distance ~/ moduleSize;
    final int tlblCentersDimension =
        (topLeft - bottomLeft).distance ~/ moduleSize;
  ...
  double calculateModuleSize(
      Offset topLeft, Offset topRight, Offset bottomLeft) {
    // Take the average
  ...
  double sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX, int toY) {
    ...
    if (state == 2) {
      return (Offset((toX + xstep).toDouble(), toY.toDouble()) -
              Offset(fromX.toDouble(), fromY.toDouble()))
          .distance;
    }
